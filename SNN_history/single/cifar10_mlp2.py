...
# 构建两层 MLP + LIF 的 SNN
###############################################################################
def build_mlp(tau: float, hidden_size: int):
    """
    两层网络结构 (CIFAR-10 输入为 3*32*32=3072):
    1) Flatten
    2) Linear(3072 -> hidden_size) + LIF
    3) Linear(hidden_size -> 10) + LIF
    """
    in_features = 3 * 32 * 32  # CIFAR-10 的单张图像展平后大小

    net = nn.Sequential(
        layer.Flatten(),
        layer.Linear(in_features, hidden_size, bias=False),
...
def objective(trial: optuna.Trial):
    ...
    # 构建网络并移动到计算设备
    net = build_mlp(tau, hidden_size=hidden_size).to(device=DEVICE)
