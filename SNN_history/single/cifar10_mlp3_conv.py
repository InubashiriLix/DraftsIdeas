...
###############################################################################
# 构建前后卷积层和三个 LIF 层之后再加卷积的 SNN
###############################################################################
def build_convnet_with_three_lif_and_conv(tau: float, hidden_size: int):
    """
    网络结构:
    1) Conv2d(3 -> 32) -> ReLU -> MaxPool2d -> LIF
    2) Conv2d(32 -> 64) -> ReLU -> MaxPool2d -> LIF
    3) Conv2d(64 -> 128) -> ReLU -> MaxPool2d -> LIF
    4) Flatten
    5) Linear(128*8*8 -> hidden_size) + LIF
    6) Linear(hidden_size -> 10) + LIF
    """
    ...
    net = nn.Sequential(
        ...
        neuron.LIFNode(tau=tau, surrogate_function=surrogate.ATan()),  # 第一个LIF层
        # 第二个卷积层
        layer.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),
        nn.ReLU(),
        nn.MaxPool2d(kernel_size=2, stride=2),  # 输出 64x8x8
        neuron.LIFNode(tau=tau, surrogate_function=surrogate.ATan()),  # 第二个LIF层
        # 第三个卷积层
        layer.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),
        nn.ReLU(),
        nn.MaxPool2d(kernel_size=2, stride=2),  # 输出 128x4x4
        neuron.LIFNode(tau=tau, surrogate_function=surrogate.ATan()),  # 第三个LIF层
        layer.Flatten(),
        layer.Linear(128 * 4 * 4, hidden_size, bias=False),  # Flatten 后的大小
        neuron.LIFNode(tau=tau, surrogate_function=surrogate.ATan()),  # LIF层
        layer.Linear(hidden_size, 10, bias=False),
        neuron.LIFNode(tau=tau, surrogate_function=surrogate.ATan()),  # 最后一层LIF
    )
    return net
...
def train_one_epoch(net, train_loader, encoder, optimizer, T, device):
    ...
    for img, label in train_loader:
        ...
        out_fr = 0.0
        for _ in range(T):
            spike_img = encoder(img)  # 将原始图像编码成脉冲
...
def objective(trial: optuna.Trial):
    # 定义超参数搜索空间
    T = trial.suggest_int("T", 5, 20, step=5)  # 仿真步数
    lr = trial.suggest_float("lr", 1e-4, 1e-2, log=True)  # 学习率
    tau = trial.suggest_float("tau", 1.0, 4.0)  # LIF 的时间常数
    ...
    # 构建网络并移动到计算设备
    net = build_convnet_with_three_lif_and_conv(tau, hidden_size=hidden_size).to(
        device=DEVICE
    )
