...
###############################################################################
# 构建前后都有卷积层且包含三个LIF层的 SNN
###############################################################################
def build_convnet_with_three_lif_layers(tau: float, hidden_size: int):
    """
    网络结构:
    1) Conv2d(3 -> 32) -> ReLU -> MaxPool2d -> LIF
    2) Conv2d(32 -> 64) -> ReLU -> MaxPool2d -> LIF
    3) Flatten
    4) Linear(64*8*8 -> hidden_size) + LIF
    5) Linear(hidden_size -> 10) + LIF
    """
    ...
    net = nn.Sequential(
        ...
        layer.Flatten(),
        layer.Linear(64 * 8 * 8, hidden_size, bias=False),  # Flatten 后的大小
...
def objective(trial: optuna.Trial):
    ...
    # 构建网络并移动到计算设备
    net = build_convnet_with_three_lif_layers(tau, hidden_size=hidden_size).to(
        device=DEVICE
    )
