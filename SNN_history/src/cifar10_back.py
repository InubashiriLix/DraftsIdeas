...
# ========== 5. 4 卷积层 + Dropout 的 SNN 网络 ==========
class SurrogateBinarySNN_4ConvDropout(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(self.time_steps):
            ...
            # 全连接
            flatten = d4_out.view(batch_size, -1)
...
if __name__ == "__main__":
    model = SurrogateBinarySNN_4ConvDropout(
        time_steps=TIME_STEPS, drop_prob=DROP_PROB
    ).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=LR)
    # 如果还想用 weight decay 抑制过拟合，可以:
