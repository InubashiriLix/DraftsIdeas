...
# ==============================
# 常见的数据增强和归一化
# 官方均值: (0.4914, 0.4822, 0.4465), 方差: (0.2023, 0.1994, 0.2010)
...
class SNN_CNN(nn.Module):
    ...
    def forward(self, x, time_window=10):
        ...
        for _ in range(time_window):
            ...
            # 累加脉冲
            # 注意，这里示例直接把最后一层当作静态输出(没加 LIF)，也可以再接一个 LIFNeuron
            # 如果想让输出层也发脉冲，需要再增加一个 LIF
            out_spike_sum += out  # 这里把输出当做连续值累加(也可再接一层 spike)
