import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader
import torchvision.transforms as transforms
import torchvision.datasets as datasets

# ========== 0. 基础配置 (超参数) ==========
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Using device:", device)

# 超参数
TIME_STEPS = 16  # STBP中，我们会在时域上展开这么多步
BATCH_SIZE = 64
EPOCHS = 200
LR = 1e-3
DROP_PROB = 0.01  # Dropout 概率
THRESH = 0.9  # LIF 发放阈值
DECAY = 0.3  # LIF 膜电位衰减

# ========== 1. 数据加载 (CIFAR-10 灰度 + Normalize) ==========
transform_train = transforms.Compose(
    [
        transforms.Grayscale(num_output_channels=1),
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,)),
    ]
)

transform_test = transforms.Compose(
    [
        transforms.Grayscale(num_output_channels=1),
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,)),
    ]
)

train_dataset = datasets.CIFAR10(
    root="./data", train=True, download=True, transform=transform_train
)
test_dataset = datasets.CIFAR10(
    root="./data", train=False, download=True, transform=transform_test
)

train_loader = DataLoader(
    train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=2
)
test_loader = DataLoader(
    test_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=2
)


# ========== 2. (示例) 二进制脉冲编码函数 ==========
#   在 STBP 中，我们需要把静态图像转换为随时间变化的脉冲序列。
#   这里仍然使用你原先的 binary_encode 做8位/16位二进制展开，仅作示例。
def binary_encode(tensor, time_steps=TIME_STEPS):
    """
    将像素值(范围-1~1或0~1)转换为多比特的二进制脉冲序列, 每个像素被拆成若干bit在不同时间步发放。
    这是一种简单的离散化编码方式。
    """
    temp = (tensor + 1.0) / 2.0 * 255.0
    temp = temp.clamp(0, 255).byte()

    batch_size, channels, H, W = temp.shape
    encoded = torch.zeros(batch_size, time_steps, channels, H, W, device=temp.device)

    for t in range(min(time_steps, 8)):
        bit_shift = 7 - t
        bit = (temp >> bit_shift) & 1
        encoded[:, t] = bit.float()

    return encoded


# ========== 3. 替代梯度(线性窗) ==========


class STBPSurrogate(torch.autograd.Function):
    """
    用于 STBP 的替代梯度函数。在 forward 时就是 Heaviside (x>0)；
    backward 时则用一个近似的线性窗口来传递梯度。
    """

    @staticmethod
    def forward(ctx, x):
        out = (x > 0).float()
        ctx.save_for_backward(x)
        return out

    @staticmethod
    def backward(ctx, grad_output):
        (x,) = ctx.saved_tensors
        alpha = 1.0
        mask = (x.abs() < 1.0).float()  # 简单的线性近似，只在-1~1范围内传梯度
        grad_input = grad_output * alpha * mask
        return grad_input


# ========== 4. LIF 神经元 (带 STBP 替代梯度) ==========
class LIFNeuron_STBP(nn.Module):
    """
    这里用 LIF 动力学:
      mem(t+1) = mem(t)*(1-decay) + Input
      spike(t+1) = H( mem(t+1) - threshold )
      if spike==1, mem(t+1) = 0  (或 mem减阈值,本例中置0)
    并用 STBPSurrogate 来近似不可导的 Heaviside。
    """

    def __init__(self, threshold=1.0, decay=0.2):
        super().__init__()
        self.threshold = threshold
        self.decay = decay
        self.spike_fn = STBPSurrogate.apply

    def forward(self, x, mem):
        # 更新膜电位
        mem = mem + x - self.decay * mem
        # 产生脉冲 (硬阈值)
        spike = self.spike_fn(mem - self.threshold)
        # 重置电位
        mem = mem * (1.0 - spike)
        return spike, mem


# ========== 5. (示例) 4 卷积层 + Dropout 的 SNN (STBP 版本) ==========
class STBP_4ConvSNN(nn.Module):
    """
    使用 STBP 训练的脉冲神经网络示例: 4层卷积 + 1层全连接 + LIF 神经元。
    """

    def __init__(self, time_steps=8, drop_prob=0.3):
        super().__init__()
        self.time_steps = time_steps

        # (1) 空间层: 4层卷积 + 池化 + Dropout
        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, padding=1)
        self.pool1 = nn.MaxPool2d(2, 2)
        self.drop1 = nn.Dropout2d(p=drop_prob)

        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.pool2 = nn.MaxPool2d(2, 2)
        self.drop2 = nn.Dropout2d(p=drop_prob)

        self.conv3 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.pool3 = nn.MaxPool2d(2, 2)
        self.drop3 = nn.Dropout2d(p=drop_prob)

        self.conv4 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.pool4 = nn.MaxPool2d(2, 2)
        self.drop4 = nn.Dropout2d(p=drop_prob)

        # (2) 全连接层 (读出层)
        self.fc = nn.Linear(256 * 2 * 2, 10)

        # (3) 时间层: LIF 神经元 (用于STBP的时域展开)
        self.lif1 = LIFNeuron_STBP(threshold=THRESH, decay=DECAY)
        self.lif2 = LIFNeuron_STBP(threshold=THRESH, decay=DECAY)
        self.lif3 = LIFNeuron_STBP(threshold=THRESH, decay=DECAY)
        self.lif4 = LIFNeuron_STBP(threshold=THRESH, decay=DECAY)
        self.lif_fc = LIFNeuron_STBP(threshold=THRESH, decay=DECAY)

    def forward(self, x):
        """
        x.shape = [batch_size, time_steps, channels=1, H=32, W=32]
        在时间维度上做展开: for t in range(self.time_steps):
          1) 提取 t 时刻的输入脉冲
          2) 过卷积 -> LIF -> pool -> dropout
          3) 重复4层
          4) 全连接 -> LIF
        最终将所有时间步的输出累加/平均，得到最终结果。
        """
        batch_size = x.size(0)

        # 对各层的膜电位初始化
        mem1 = torch.zeros(batch_size, 32, 32, 32, device=x.device)
        mem2 = torch.zeros(batch_size, 64, 16, 16, device=x.device)
        mem3 = torch.zeros(batch_size, 128, 8, 8, device=x.device)
        mem4 = torch.zeros(batch_size, 256, 4, 4, device=x.device)
        mem_fc = torch.zeros(batch_size, 10, device=x.device)

        output_sum = torch.zeros(batch_size, 10, device=x.device)

        # ===== 在时间上展开 =====
        for t in range(self.time_steps):
            current_input = x[:, t]  # [batch_size, 1, H=32, W=32]

            # 第1层: conv -> LIF -> pool -> dropout
            c1_out = self.conv1(current_input)
            s1, mem1 = self.lif1(c1_out, mem1)
            p1_out = self.pool1(s1)
            d1_out = self.drop1(p1_out)

            # 第2层
            c2_out = self.conv2(d1_out)
            s2, mem2 = self.lif2(c2_out, mem2)
            p2_out = self.pool2(s2)
            d2_out = self.drop2(p2_out)

            # 第3层
            c3_out = self.conv3(d2_out)
            s3, mem3 = self.lif3(c3_out, mem3)
            p3_out = self.pool3(s3)
            d3_out = self.drop3(p3_out)

            # 第4层
            c4_out = self.conv4(d3_out)
            s4, mem4 = self.lif4(c4_out, mem4)
            p4_out = self.pool4(s4)
            d4_out = self.drop4(p4_out)

            # 全连接
            flatten = d4_out.view(batch_size, -1)  # [batch_size, 256*2*2]
            fc_out = self.fc(flatten)  # [batch_size, 10]
            s_fc, mem_fc = self.lif_fc(fc_out, mem_fc)  # [batch_size, 10]

            # 累加每个时刻的输出
            output_sum += fc_out

        # 将所有时间步的输出取平均
        return output_sum / self.time_steps


# ========== 6. 训练 & 测试函数 (STBP) ==========
def train_one_epoch(model, loader, optimizer, criterion, epoch):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0

    for batch_idx, (data, target) in enumerate(loader):
        data, target = data.to(device), target.to(device)
        # 编码: 把静态图像变成 [batch, TIME_STEPS, 1, H, W] 的时序脉冲
        encoded = binary_encode(data, time_steps=TIME_STEPS)

        optimizer.zero_grad()

        # 前向: 在时域上展开 + Surrogate Gradient
        outputs = model(encoded)

        # 计算 loss
        loss = criterion(outputs, target)
        # 反向传播 (就是 STBP：时域展开 + 替代梯度)
        loss.backward()
        optimizer.step()

        # 统计指标
        running_loss += loss.item()
        _, predicted = outputs.max(1)
        correct += predicted.eq(target).sum().item()
        total += target.size(0)

    acc = 100.0 * correct / total
    print(
        f"[Epoch {epoch}] Train Loss: {running_loss / len(loader):.4f}, Train Acc: {acc:.2f}%"
    )


def test_model(model, loader, criterion, epoch):
    model.eval()
    loss_total = 0.0
    correct = 0
    total = 0

    with torch.no_grad():
        for data, target in loader:
            data, target = data.to(device), target.to(device)
            encoded = binary_encode(data, time_steps=TIME_STEPS)
            outputs = model(encoded)
            loss = criterion(outputs, target)
            loss_total += loss.item()

            _, predicted = outputs.max(1)
            correct += predicted.eq(target).sum().item()
            total += target.size(0)

    acc = 100.0 * correct / total
    print(
        f"[Epoch {epoch}] Test  Loss: {loss_total / len(loader):.4f}, Test  Acc: {acc:.2f}%"
    )
    return acc


# ========== 7. 主函数 (完整 STBP 训练流程) ==========
if __name__ == "__main__":
    # 构建网络
    model = STBP_4ConvSNN(time_steps=TIME_STEPS, drop_prob=DROP_PROB).to(device)

    # 损失函数 & 优化器 (依然可以使用常见的 cross-entropy + Adam)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=LR)
    # 也可以选择 weight_decay: optimizer = optim.Adam(model.parameters(), lr=LR, weight_decay=1e-4)

    best_acc = 0.0  # 记录最好测试精度
    best_epoch = 0  # 记录是哪一个 epoch

    # 训练 EPOCHS 个周期
    for epoch in range(1, EPOCHS + 1):
        train_one_epoch(model, train_loader, optimizer, criterion, epoch)
        test_acc = test_model(model, test_loader, criterion, epoch)

        # 如果本 epoch 测试集准确率优于之前最好纪录，则保存
        if test_acc > best_acc:
            best_acc = test_acc
            best_epoch = epoch
            torch.save(model.state_dict(), "best_model_stbp.pth")
            print(
                f"==> New best accuracy: {best_acc:.2f}% at epoch {best_epoch}, model saved!"
            )

    print(
        f"Training completed. Best test accuracy: {best_acc:.2f}% (epoch {best_epoch})."
    )
