...
void drawOcean(float timeSec) {
  // 海在海堤外侧：z 负方向（无底部地面，只有水面）
  const float OCEAN_HALF_X =
      450.0f; // 海洋宽度匹配Dam (Dam: 9×100=900, halfWidth=450)
  const float OCEAN_NEAR_Z = -60.0f; // 从Dam坡脚开始 (matches OCEAN_SLOPE_Z)
  const float DX = 20.0f;
  const float DZ = 20.0f;
  const int COLS = 45; // x 方向网格数 (450×2/20=45) - restored to match width
  const int ROWS = 40; // z 方向网格数（海洋深度约500）

  float baseY = 5.0f; // 海平面基准（波谷在更低） - Raised from -3.0f

  glEnable(GL_BLEND);
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  glColor4f(0.12f, 0.28f, 0.40f, 0.85f); // 半透明蓝绿色海面
...
const float WALL_SEG_LENGTH = 100.0f; // 调整单段长度
const int WALL_SEG_COUNT = 4;    // 每侧 4 段，合计 9 段，总宽度 900 ≈ 城市宽度
