...
struct WeatherState {
  ...
  float fogDensity = 0.0005f; // 0.0 - 0.01
...
bool isInBounds(float x, float y, float z) {
  ...
  if (x < gBounds.minX || x > gBounds.maxX) return false;
  if (y < gBounds.minY || y > gBounds.maxY) return false;
  if (z < gBounds.minZ || z > gBounds.maxZ) return false;
  
  // Additional area-specific height checks for terrain following
  
  // Desert/Mesa area (world Z: -1800 to -1250)
  if (z <= -1250.0f) {
    if (y < 40.0f) return false; // Above highest dune/mesa (raised from 35)
  }
  
  // Beach transition area (world Z: -1250 to -1130)
  else if (z <= -1130.0f) {
    if (y < 30.0f) return false; // Above beach/dune transition
  }
  
  // Ocean area (world Z: -1130 to -630)
  else if (z <= -630.0f) {
    if (y < 12.0f) return false; // Above water surface
  }
  
  // Dam area (world Z: -630 to -500)
  else if (z <= -500.0f) {
    if (y < 10.0f) return false; // Above dam structure
  }
  
  // City area (world Z: -400 to +400)
  else if (z <= 400.0f) {
    if (y < 5.0f) return false; // Above city ground
  }
  
  // Stella area (world Z: +400 to +650)
  else {
    if (y < 5.0f) return false; // Above ground
  }
...
void setupDayNightLighting() {
  ...
  GLfloat clearColor[4] = {
    ...
    (0.05f + 0.18f * dayFactor + 0.20f * nightFactor) * cloudDarkening,
    1.0f};
  ...
  if (gWeather.fogDensity > 0.0001f) {
    glEnable(GL_FOG);
    GLfloat fogColor[] = {clearColor[0] * 1.2f, clearColor[1] * 1.2f, clearColor[2] * 1.2f, 1.0f};
  ...
  GLfloat sunDiffuse[] = {
    ...
    (0.4f + 0.6f * dayFactor) * lightReduction,
    (0.45f + 0.55f * dayFactor) * lightReduction, 
    1.0f};
...
void drawClouds() {
  if (gWeather.cloudCoverage < 0.01f) return;
...
void updateRain(float dt) {
  if (!gWeather.rainEnabled) return;
...
void drawRain() {
  if (!gWeather.rainEnabled || gWeather.rainIntensity < 0.01f) return;
  ...
  for (int i = 0; i < drawCount && i < (int)gRain.size(); ++i) {
    ...
    glVertex3f(r.x, r.y - 3.0f * gWeather.rainIntensity, r.z); // Rain streak length
  }
  glEnd();
...
void keyboardDown(unsigned char key, int, int) {
  ...
  case 'H':
    ...
    printf("F / G: Decrease/increase fog density\n");
...
void initIntegratedGL() {
  ...
  printf("F / G: Decrease/increase fog density\n");
  printf("C / V: Decrease/increase cloud coverage\n");
  printf("H: Show help and current position\n");
  printf("\n--- Movement Boundaries ---\n");
  printf("X: %.0f to %.0f\n", gBounds.minX, gBounds.maxX);
  printf("Y: %.0f to %.0f (ground to clouds)\n", gBounds.minY, gBounds.maxY);
  printf("Z: %.0f to %.0f\n", gBounds.minZ, gBounds.maxZ);
...
// TODO: raise the start point of snow in the Stella area
// TODO: add the Buffer area of stella
// TODO: unify the length of all the items: Desert + sea + Dam. // 450
