...
const float MESA_HALF_SIZE = 200.0f; // 高地半宽（reduced from 260.0f）
const float MESA_BUMP_HEIGHT =
    70.0f; // Mesa mound 额外高度 (reduced from 80.0f)
const float DUNE_BASE_Y =
    30.0f; // 沙面基准高度 (Raised higher for more dramatic terrain)
...
private:
  struct DebrisChunk {
    ...
    bool isBox; // 存储形状类型：true=盒子，false=圆柱
  };
  ...
  void initDebris() {
    ...
    for (int i = 0; i < NUM_DEBRIS; ++i) {
      ...
      d.isBox = (frand(0.0f, 1.0f) < 0.7f); // 70% 是盒子，30% 是圆柱
      debris_.push_back(d);
    }
  }
